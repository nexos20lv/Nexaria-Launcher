...
public class NewsCard extends JPanel {
    ...
    public NewsCard(RSSFeedParser.NewsItem item) {
        ...
        if (item.getImageUrl() != null) {
            new Thread(() -> {
                try {
                    java.net.URL url = new java.net.URI(item.getImageUrl()).toURL();
                    ...
                    // Ignore error, keep default background
                }
            }).start();
        }
    ...
    protected void paintComponent(Graphics g) {
        ...
        if (backgroundImage != null) {
            g2d.setClip(new java.awt.geom.RoundRectangle2D.Double(0, 0, getWidth(), getHeight(), CORNER_RADIUS,
                    CORNER_RADIUS));
        ...
        // 2. Gradient Overlay (Darker at bottom for text readability)
        GradientPaint overlay;
        if (backgroundImage != null) {
            overlay = new GradientPaint(
                    0, 0, new Color(20, 10, 30, 100),
                    0, getHeight(), new Color(10, 5, 20, 240));
        ...
        // 3. Hover Border
        if (isHovered) {
            g2d.setColor(DesignConstants.PURPLE_ACCENT);
        ...
        g2d.drawRoundRect(
                1, 1, getWidth() - 2, getHeight() - 2, CORNER_RADIUS, CORNER_RADIUS);
